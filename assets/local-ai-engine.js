(function(){
'use strict';
const AI={version:'27.4.1',records:[],sources:[],risk:[],analysis:null,recommendations:[],ready:false};
const APP_MAP=[
...
function build(){if(document.getElementById('localAiFab'))return;
 const wrap=document.createElement('div');wrap.id='localAiLauncher';wrap.className='local-ai-launcher';
 const fab=document.createElement('button');fab.id='localAiFab';fab.className='local-ai-fab';fab.setAttribute('aria-label','فتح أدوات الذكاء الاصطناعي');fab.setAttribute('aria-expanded','false');fab.innerHTML='<span>🤖</span><b>الذكاء الاصطناعي</b>';
 const menu=document.createElement('div');menu.id='localAiMenu';menu.className='local-ai-menu';menu.innerHTML='<button data-ai-action="executive"><span>📈</span><b>التقرير التنفيذي</b></button><button data-ai-action="plan"><span>🎯</span><b>خطة العمل</b></button><button data-ai-action="analytics"><span>📊</span><b>التحليل الذكي</b></button><button data-ai-action="assistant"><span>🤖</span><b>مساعد الإدارة</b></button>';
 wrap.append(menu,fab);
 const closeMenu=()=>{menu.classList.remove('open');fab.setAttribute('aria-expanded','false')};
 fab.onclick=e=>{e.stopPropagation();const open=menu.classList.toggle('open');fab.setAttribute('aria-expanded',open?'true':'false')};
 menu.onclick=e=>{const b=e.target.closest('[data-ai-action]');if(!b)return;closeMenu();const action=b.dataset.aiAction;if(action==='executive')openExecutive();else if(action==='plan')openActionPlan();else if(action==='analytics')openAnalytics();else toggle(true)};
 ...
 document.body.append(wrap,box);box.querySelector('#aiClose').onclick=()=>toggle(false);box.querySelectorAll('.local-ai-chips button').forEach(b=>b.onclick=()=>ask(b.textContent));box.querySelector('#aiForm').onsubmit=e=>{e.preventDefault();ask(box.querySelector('#aiInput').value);box.querySelector('#aiInput').value=''};refreshMetrics();
}
...
window.LocalEnterpriseAI={load,summary,answer,toggle,openAnalytics,openActionPlan,openExecutive,executiveNarrative,smartRecordSummary,systemAnalysis,recommendationText,version:AI.version,
 getRisk:()=>{load();return AI.risk},
 getExecutiveSnapshot:()=>{load();return executiveSnapshot()}
};
