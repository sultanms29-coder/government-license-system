...
(() => {
  ...
  // منع التضمين داخل موقع خارجي مختلف (حماية من هجمات clickjacking) — مع السماح
  // بالتضمين من نفس أصل المنصة (نفس النطاق)، لأن الصفحة الرئيسية تستخدم إطارات
  // مخفية من نفس الأصل لمزامنة كل الأنظمة بضغطة واحدة؛ أي تضمين من نطاق مختلف
  // فعليًا لا يزال يُرفض فورًا كما كان.
  try {
    if (window.top !== window.self) {
      let sameOrigin = false;
      try { sameOrigin = window.top.location.origin === window.self.location.origin; } catch (_) { sameOrigin = false; }
      if (!sameOrigin) window.top.location = window.self.location.href;
    }
  ...
  function lockSession() {
    ...
    if (typeof window.SCC_FORCE_LOGIN_SCREEN === 'function') window.SCC_FORCE_LOGIN_SCREEN();
