/* platform-sync-hub.js — v28.18.56
...
(()=>{'use strict';
 ...
 function applyRow(row){if(!row?.workspace_id||!WORKSPACES[row.workspace_id]||!row.payload)return false;const m=WORKSPACES[row.workspace_id],rev=Number(row.revision||0),mm=meta(),prev=Number(mm.revisions?.[row.workspace_id]||0);if(rev&&prev&&rev<prev)return false;
  try{
   ...
   else{const local=parse(localStorage.getItem(m.cache));const pending=localStorage.getItem('scc-pending-cloud-push:'+row.workspace_id)==='1';const merged=pending?safeMerge(row.payload,local):row.payload;localStorage.setItem(m.cache,JSON.stringify(merged));}
