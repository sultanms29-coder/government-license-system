/* core-updater.js — تحديث موحّد عبر كل صفحات المنصة (الرئيسية + الأنظمة السبعة)
...
#coreUpdateBar.show{display:none!important}
...
@media(max-width:600px){#coreUpdateBar{font-size:11px;padding:8px 10px}#coreUpdateBar .cu-btn{padding:5px 10px}}
  ...
  async function check() {
    try {
      ...
      if (compare(meta.version, installed) > 0) {
        // تحديث صامت: لا نعرض أي رسالة أو شريط تحديث للمستخدم.
        // نحدّث ملفات التطبيق/Service Worker فقط ونحافظ على جميع بيانات localStorage.
        const flag='scc-silent-update-'+meta.version;
        let already=false;try{already=sessionStorage.getItem(flag)==='1'}catch(_){}
        if(!already){
          try{sessionStorage.setItem(flag,'1')}catch(_){}
          await clearApplicationCache();
          localStorage.setItem(INSTALLED_KEY, meta.version);
          // تحديث ملفات التطبيق في الخلفية فقط. لا نعيد تحميل الصفحة أمام المستخدم؛
          // تُستخدم النسخة الجديدة عند التنقل الطبيعي أو الفتح التالي للمنصة.
          try{window.dispatchEvent(new CustomEvent('scc:background-version-ready',{detail:{version:meta.version}}))}catch(_){}
        }
      } else {
        hideBar();
      }
  ...
  function watchForStuckLoadingScreen() {
    ...
    setTimeout(() => {
      ...
      if (!reloadedAlready) {
        ...
        // تكرر التعليق حتى بعد إعادة تحميل واحدة — أخفِ شاشة التحميل قسرًا كحل أخير
  ...
  function mountTopbarAiButton() {
    // فقط داخل الأنظمة السبعة (وليس الصفحة الرئيسية، حيث الوصول أصبح من الإعدادات).
    if (!location.pathname.includes('/apps/')) return;
    let tries = 0;
    const tryMount = () => {
      tries++;
  ...
  function seedMasterCredentials() {
    ...
    const FALLBACK = {
      projectUrl: 'https://jpoijjlgrfsctritfpbrp.supabase.co',
    ...
    const configs = {
      ...
      'srco-daily-work-supabase-cfg':'alandiyah-daily-work',
      'srco-shomoos-sync-cfg':'alandiyah-shomoos'
    };
    try {
      let master = null;
