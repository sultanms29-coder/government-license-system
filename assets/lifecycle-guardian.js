...
(() => {
  ...
  function getValidUser() {
    ...
    const candidate = session || remembered;
    if (!candidate) return null;
    const norm = v => String(v || '').trim().replace(/\s+/g, ' ');
    const users = getUsers();
    // Device-safe identity recovery: IDs can legitimately change after an admin/user migration.
    // Username/name are stable identity fallbacks and prevent one stale device from being locked out.
    const match = users.find(u => u && candidate.id && u.id === candidate.id)
      || users.find(u => u && candidate.username && norm(u.username) === norm(candidate.username))
      || users.find(u => u && candidate.name && norm(u.name) === norm(candidate.name));
    if (!match) return null;
    const safe = { ...match };
    delete safe.passwordHash;
    try {
      sessionStorage.setItem(KEYS.current, JSON.stringify(safe));
      if (remembered) localStorage.setItem(KEYS.remember, JSON.stringify(safe));
    } catch (_) {}
    return safe;
  }
