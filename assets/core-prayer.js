...
.core-prayer-words{min-height:150px;display:grid;place-items:center;align-content:center;gap:5px;margin:12px auto 18px;padding:18px;border-block:1px solid rgba(219,190,117,.18);overflow:hidden}.core-prayer-word{transition:opacity .5s ease,transform .5s ease,filter .5s ease}.core-prayer-word.prev,.core-prayer-word.next{font-size:14px;color:#b9c7bf;opacity:.35;filter:blur(.15px)}.core-prayer-word.prev{transform:translateY(-5px) scale(.94)}.core-prayer-word.next{transform:translateY(5px) scale(.94)}.core-prayer-word.current{font-size:clamp(25px,5vw,42px);font-weight:900;color:#f7e6ae;text-shadow:0 0 28px rgba(229,194,104,.3);animation:prayerWordIn .72s ease both}
...
@keyframes prayerWordIn{from{opacity:0;transform:translateY(13px) scale(.94);filter:blur(5px)}to{opacity:1;transform:none;filter:none}}
@media(max-width:600px){.core-prayer-card{padding:30px 18px 24px;border-radius:24px}.core-prayer-message{font-size:14px}.core-prayer-mosque{font-size:46px}}
  ...
  function ensureOverlay() {
    ...
    overlay.innerHTML = `<div class="core-prayer-card">
      ...
      <div class="core-prayer-words" id="corePrayerWords" aria-live="polite">
        <div class="core-prayer-word prev" id="corePrayerWordPrev"></div>
        <div class="core-prayer-word current" id="corePrayerWordCurrent">الله أكبر، الله أكبر</div>
        <div class="core-prayer-word next" id="corePrayerWordNext">الله أكبر، الله أكبر</div>
      </div>
  ...
  function updateAdhanWords(elapsed,total) {
    const usable=Math.max(1,Number(total)||231);
    const index=Math.min(ADHAN_LINES.length-1,Math.floor(Math.max(0,elapsed)/usable*ADHAN_LINES.length));
    const current=document.getElementById('corePrayerWordCurrent'),prev=document.getElementById('corePrayerWordPrev'),next=document.getElementById('corePrayerWordNext');
    if(!current||current.dataset.index===String(index))return;
    current.dataset.index=String(index);current.textContent=ADHAN_LINES[index];prev.textContent=index?ADHAN_LINES[index-1]:'';next.textContent=ADHAN_LINES[index+1]||'';
    current.style.animation='none';requestAnimationFrame(()=>{current.style.animation='prayerWordIn .72s ease both'});
  }
  ...
  async function startPrayerMode(name, key) {
    ...
    const update = () => {
      ...
      updateAdhanWords(elapsed,total);
    };
  ...
  // مهم: لا نحاول تشغيل ملف الأذان عند أول لمسة أو عند فتح أيقونة.
  // بعض أجهزة iPhone تتجاهل volume=0 لحظيًا، فكان ذلك يؤدي إلى سماع بداية الأذان
  // عند كل انتقال بين صفحات المنصة. نسجل التفاعل فقط، ونشغّل الصوت حصريًا
  // عند دخول وقت الصلاة أو عند الضغط الصريح على زر الاختبار.
  function registerUserInteraction() {
    audioUnlocked = true;
  }
  ['pointerdown','keydown','touchstart'].forEach(ev => window.addEventListener(ev, registerUserInteraction, { once:true, passive:true }));
  ...
  function checkNow(timings) {
    ...
    for (const key of ORDER) {
      ...
      if (t && t === hhmm && !notified.includes(key)) {
        // نسجّل الصلاة أولًا قبل تشغيل الشاشة؛ حتى لا تعيد الصفحة الجديدة تشغيل
        // الأذان إذا انتقل المستخدم إلى نظام آخر خلال نفس الدقيقة.
        notified.push(key);
        try { localStorage.setItem(NOTIFIED_KEY, JSON.stringify({ day: todayKey(), set: notified })); } catch (_) {}
  ...
  function format12Hour(value, en=false) {
    const m=String(value||'').match(/(\d{1,2}):(\d{2})/);if(!m)return String(value||'').slice(0,5);
    const h=Number(m[1]),min=m[2],suffix=h<12?(en?'AM':'ص'):(en?'PM':'م');
  ...
  function renderWidget(timings) {
    ...
    host.innerHTML = ORDER.map(k => `<div class="pt-item"><b>${format12Hour(timings[k],en)}</b><span>${en?enNames[k]:PRAYER_NAMES[k]}</span></div>`).join('');
  }

  function weatherMeta(code){
    const sun=`<svg viewBox="0 0 48 48" aria-hidden="true"><circle cx="24" cy="24" r="8" fill="currentColor"/><g stroke="currentColor" stroke-width="3" stroke-linecap="round"><path d="M24 5v5M24 38v5M5 24h5M38 24h5M10.5 10.5l3.7 3.7M33.8 33.8l3.7 3.7M37.5 10.5l-3.7 3.7M14.2 33.8l-3.7 3.7"/></g></svg>`;
    const cloud=`<svg viewBox="0 0 48 48" aria-hidden="true"><path d="M13 35h24a8 8 0 0 0 1-15.9A12 12 0 0 0 15.4 18 8.5 8.5 0 0 0 13 35Z" fill="currentColor"/></svg>`;
    const part=`<svg viewBox="0 0 48 48" aria-hidden="true"><circle cx="18" cy="17" r="7" fill="currentColor" opacity=".9"/><path d="M13 36h25a8 8 0 0 0 1-15.9A12 12 0 0 0 16.5 19 8.5 8.5 0 0 0 13 36Z" fill="currentColor"/></svg>`;
    const fog=`<svg viewBox="0 0 48 48" aria-hidden="true"><g stroke="currentColor" stroke-width="4" stroke-linecap="round"><path d="M8 17h32M5 25h30M12 33h31"/></g></svg>`;
    const rain=`<svg viewBox="0 0 48 48" aria-hidden="true"><path d="M12 30h25a8 8 0 0 0 1-15.8A12 12 0 0 0 15.5 13 8.5 8.5 0 0 0 12 30Z" fill="currentColor"/><g stroke="currentColor" stroke-width="3" stroke-linecap="round"><path d="M17 35l-2 5M26 35l-2 5M35 35l-2 5"/></g></svg>`;
    const storm=`<svg viewBox="0 0 48 48" aria-hidden="true"><path d="M11 29h26a8 8 0 0 0 1-15.8A12 12 0 0 0 15 12 8.5 8.5 0 0 0 11 29Z" fill="currentColor"/><path d="M25 29 19 39h6l-2 7 9-12h-6l3-5Z" fill="#f1c84b"/></svg>`;
    const snow=`<svg viewBox="0 0 48 48" aria-hidden="true"><path d="M11 28h26a8 8 0 0 0 1-15.8A12 12 0 0 0 15 11 8.5 8.5 0 0 0 11 28Z" fill="currentColor"/><g fill="currentColor"><circle cx="17" cy="36" r="2"/><circle cx="25" cy="39" r="2"/><circle cx="34" cy="35" r="2"/></g></svg>`;
    if(code===0)return {svg:sun,label:'صحو'};if(code===1||code===2)return {svg:part,label:'غائم جزئياً'};if(code===3)return {svg:cloud,label:'غائم'};if(code===45||code===48)return {svg:fog,label:'ضباب'};if([51,53,55,56,57,61,63,65,66,67,80,81,82].includes(code))return {svg:rain,label:'أمطار'};if([71,73,75,77,85,86].includes(code))return {svg:snow,label:'ثلوج'};if([95,96,99].includes(code))return {svg:storm,label:'عواصف رعدية'};return {svg:cloud,label:'حالة الطقس'};
  }
  async function fetchWeather() {
    const host = document.getElementById('weatherWidget');if (!host) return;
    try {const r=await fetch('https://api.open-meteo.com/v1/forecast?latitude=24.7136&longitude=46.6753&current_weather=true',{cache:'no-store'});if(!r.ok)throw new Error('bad response');const data=await r.json(),cw=data&&data.current_weather;if(!cw)throw new Error('no weather');const meta=weatherMeta(Number(cw.weathercode));host.innerHTML=`<span class="wt-svg">${meta.svg}</span><span class="wt-copy"><b>${Math.round(cw.temperature)}°</b><small>${meta.label} · الرياض</small></span>`;host.title=`${meta.label} في الرياض — ${Math.round(cw.temperature)} درجة`;} catch (_) {}
  }

  let currentTimings = null, loadedDay = '';
  async function refreshPrayerTimes(force=false) {
    const day=todayKey();if(!force&&currentTimings&&loadedDay===day)return currentTimings;
    if(force){try{localStorage.removeItem(CACHE_KEY)}catch(_){}}
    currentTimings=await fetchTimings();loadedDay=day;renderWidget(currentTimings);checkNow(currentTimings);return currentTimings;
  }
  async function init() {
    await refreshPrayerTimes(false);
    setInterval(async()=>{if(loadedDay!==todayKey())await refreshPrayerTimes(true);checkNow(currentTimings)},30*1000);
    window.addEventListener('focus',()=>refreshPrayerTimes(false));document.addEventListener('visibilitychange',()=>{if(!document.hidden)refreshPrayerTimes(false)});
